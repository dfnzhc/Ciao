...
namespace Ciao
{
    ...
    {
        if (b.m_Type != BitmapType_2D) return Bitmap();

        const int faceSize = b.m_W / 4;

        const int w = faceSize * 3;
        const int h = faceSize * 4;

        Bitmap result(w, h, b.m_Comp, b.m_Fmt);
        ...
        const int clampW = b.m_W - 1;
        const int clampH = b.m_H - 1;
    ...
    {
        const int faceWidth = b.m_W / 3;
        const int faceHeight = b.m_H / 4;

        Bitmap cubemap(faceWidth, faceHeight, 6, b.m_Comp, b.m_Fmt);
        cubemap.m_Type = BitmapType_Cube;
        ...
        /*
         ...
         ----------------
                ...
                ------
        */
        const int pixelSize = cubemap.m_Comp * Bitmap::GetBytesPerComponent(cubemap.m_Fmt);
        ...
        for (int face = 0; face != 6; ++face) {
            for (int j = 0; j != faceHeight; ++j) {
                for (int i = 0; i != faceWidth; ++i) {
                    ...
                    memcpy(dst, src + (y * b.m_W + x) * pixelSize, pixelSize);
