...
namespace Ciao
{
                     ...
                     std::vector<std::vector<uint32_t>>& outLods)
    {
        ...
        while (targetIndicesCount > 1024 && LOD < 8)
        {
            ...
            CIAO_CORE_INFO("\n   LOD{}: {} indices {}.", int(LOD), int(numOptIndices), sloppy ? "[sloppy]" : "");
    ...
    void LoadFile(MeshData& meshData, const char* fileName)
    {
        ...
        {
            CIAO_CORE_ERROR("Unable to load '%s'\n", fileName);
            exit(255);
        }
        ...
        {
            CIAO_CORE_INFO("\nConverting meshes {}/{}...", i + 1, scene->mNumMeshes);
    ...
    void SaveDrawData(const MeshData& meshData, const char* fileName)
    {
        ...
        uint32_t vertexOffset = 0;
        for (auto i = 0 ; i < meshData.meshes_.size() ; i++)
        {
            grid.push_back(DrawData {
                .meshIndex = (uint32_t)i,
                .materialIndex = 0,
                .LOD = 0,
                .indexOffset = meshData.meshes_[i].indexOffset,
                .vertexOffset = vertexOffset,
                .transformIndex = 0
            });
            vertexOffset += meshData.meshes_[i].vertexCount;
        }
