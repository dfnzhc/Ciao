...
namespace Ciao
{
	...
	{
		fs::create_directory(Res("out_textures"));
	                           ...
	                           const std::vector<std::string>& opacityMaps)
	{
		...
		const auto newFile = std::string(Res("out_textures/")) + lowercaseString(
	...
	void MergeBistro()
	{
		...
		MeshData m1, m2;
		MeshFileHeader header1 = loadMeshData(Res("Meshes/bistro_exterior.meshes"), m1);
		MeshFileHeader header2 = loadMeshData(Res("Meshes/bistro_interior.meshes"), m2);

		std::vector<uint32_t> meshCounts = {header1.meshCount, header2.meshCount};

		loadScene(Res("Meshes/bistro_exterior.scene"), scene1);
		loadScene(Res("Meshes/bistro_interior.scene"), scene2);
		...
		loadMaterials(Res("Meshes/bistro_exterior.materials"), materials1, textureFiles1);
		loadMaterials(Res("Meshes/bistro_interior.materials"), materials2, textureFiles2);
		...
		saveMaterials(Res("Meshes/bistro_all.materials"), allMaterials, allTextures);

		CIAO_CORE_INFO("[Unmerged] scene items: {}.", (int)scene.hierarchy_.size());
		mergeScene(scene, meshData, "Foliage_Linde_Tree_Large_Orange_Leaves");
		CIAO_CORE_INFO("[Merged orange leaves] scene items: {}.", (int)scene.hierarchy_.size());
		mergeScene(scene, meshData, "Foliage_Linde_Tree_Large_Green_Leaves");
		CIAO_CORE_INFO("[Merged green leaves]  scene items: {}.", (int)scene.hierarchy_.size());
		mergeScene(scene, meshData, "Foliage_Linde_Tree_Large_Trunk");
		CIAO_CORE_INFO("[Merged trunk]  scene items: {}.", (int)scene.hierarchy_.size());

		RecalculateBoundingBoxes(meshData);

		saveMeshData(meshData, Res("Meshes/bistro_all.meshes"));
		saveScene(Res("Meshes/bistro_all.scene"), scene);
	}
}
