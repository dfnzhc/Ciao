...
namespace Ciao
{
	...
	void mergeScene(Scene& scene, MeshData& meshData, const std::string& materialName)
	{
		...
		int oldMaterial = (int)std::distance(std::begin(scene.materialNames_),
		                                     std::find(std::begin(scene.materialNames_), std::end(scene.materialNames_),
		                                               materialName));

		std::vector<uint32_t> toDelete;

		for (auto i = 0u; i < scene.hierarchy_.size(); i++)
			if (scene.meshes_.contains(i) && scene.materialForNode_.contains(i) && (scene.materialForNode_.at(i) ==
				oldMaterial))
