...
namespace Ciao
{
    ...
    void markAsChanged(Scene& scene, int node)
    {
        ...
        // 迭代方法
        // std::stack<int> stk;
        // stk.push(scene.hierarchy_[node].firstChild_);
        //
        // while (stk.size())
        // {
        ...
        //
        //         stk.push(scene.hierarchy_[s].firstChild_);
        //         stk.push(scene.hierarchy_[s].nextSibling_);
        //     }
        // }
    }
    ...
    void printChangedNodes(const Scene& scene)
    {
        ...
        {
            CIAO_CORE_TRACE("Changed at level({}):", i);

            for (const int& c : scene.changedAtThisFrame_[i])
            {
                int p = scene.hierarchy_[c].parent_;
                //scene.globalTransform_[c] = scene.globalTransform_[p] * scene.localTransform_[c];
                CIAO_CORE_TRACE(" Node {}. Parent = {}; LocalTransform: ", c, p);
                fprintfMat4(stdout, scene.localTransform_[i]);
                if (p > -1)
                {
                    CIAO_CORE_TRACE(" ParentGlobalTransform: ");
